const FORM_CHAIN_LOOKUP_SELECTOR =
  "#ext-smartaddressprotection-chain_lookup_form";
const FORM_CHAIN_LOOKUP_INPUT_SELECTOR =
  "#ext-smartaddressprotection-addr_or_tx";
const FORM_CHAIN_LOOKUP_OUTPUT_SELECTOR =
  "#ext-smartaddressprotection-chain_lookup_output";
const FORM_CHAIN_LOOKUP_DETAILS_SELECTOR =
  "#ext-smartaddressprotection-chain_lookup_details";
...
class ChainLookup {
  ...
  setupFormSubmitHandler() {
    if (document.querySelector(FORM_CHAIN_LOOKUP_SELECTOR)) {
      ...
      document
        .querySelector(FORM_CHAIN_LOOKUP_SELECTOR)
        .addEventListener("submit", async (event) => {
          event.preventDefault();

          chrome.runtime.sendMessage(
            { func: "blockchain_explorer" },
            async function (objResponse) {
              this.strBlockExplorer = objResponse.resp;

              const strInput = document.querySelector(
                FORM_CHAIN_LOOKUP_INPUT_SELECTOR
              ).value;
  ...
  async getRpcDetails() {
    if (document.querySelector(FORM_CHAIN_LOOKUP_DETAILS_SELECTOR)) {
      ...
      document.querySelector(FORM_CHAIN_LOOKUP_DETAILS_SELECTOR).innerHTML = `
                <strong>Latest Block Number:</strong> ${intBlockNumber.toLocaleString(
                  "en-US"
                )} <br />
            `;
      ...
      chrome.runtime.sendMessage({ func: "rpc_details" }, (objResponse) => {
        ...
        document.querySelector(
          FORM_CHAIN_LOOKUP_DETAILS_SELECTOR
        ).innerHTML += `
    ...
    switch (strInputType.toUpperCase()) {
      ...
      case "UNKNOWN":
        if (strInput.endsWith(".eth")) {
          document.querySelector(
            FORM_CHAIN_LOOKUP_OUTPUT_SELECTOR
          ).innerHTML += `
  ...
  async doAddressLookup(strInput) {
    let strAddressBlockie = blockies
      .create({
    ...
    objAddressDetails.eth = this.web3
      .fromWei(this.web3.eth.getBalance(strInput).toString(10), "ether")
      .toLocaleString("en-US", { maximumSignificantDigits: 5 });

    objAddressDetails.tx = parseInt(
      this.web3.eth.getTransactionCount(strInput)
    ).toLocaleString();
    objAddressDetails.contract =
      this.web3.eth.getCode(strInput) === "0x" ? "Normal (EOA)" : "Contract";

    let strAddressLookedup = strInput;
    let objLabels = new Labels();
    let objLabelledAddress = await objLabels.getLabelForAddress(
      strAddressLookedup
    );
    if (typeof objLabelledAddress !== "undefined") {
      let strLabel = `${
        objLabelledAddress.label
      } (${ChainLookup.getShortAddress(strAddressLookedup)})`;
      strAddressLookedup = objLabels.getTemplate(
        strLabel,
        objLabelledAddress.color
      );
    }
                ...
                <strong>Address:</strong> <br />
                    <span>
                        ...
                        <a target="_blank" href="${
                          this.strBlockExplorer + "/" + strInput
                        }">${strAddressLookedup}</a>
                    </span> <br />
                    <ul>
                        <li><strong>ETH:</strong> ${objAddressDetails.eth}</li>
                        <li><strong>Transactions out:</strong> ${
  ...
  async doTxLookup(strInput) {
    ...
    let objTransactionReceipt = await this.web3.eth.getTransactionReceipt(
      strInput
    );
    ...
    let strAddressFromBlockie = blockies
      .create({
    ...
    let strToAddress =
      objTransaction.to !== null
        ? objTransaction.to
        : objTransactionReceipt.contractAddress;
    let strAddressToBlockie = blockies
      .create({
    ...
    let objTxDetails = {
      eth: this.web3
        .fromWei(objTransaction.value)
        .toLocaleString("en-US", { maximumSignificantDigits: 4 }),
      eth_full: this.web3
        .fromWei(objTransaction.value)
      ...
      gas: {
        ...
        price: this.web3
          .fromWei(objTransaction.gasPrice.toString())
          .toLocaleString("en-US"),
        used: objTransactionReceipt.gasUsed,
        used_percent:
          (objTransactionReceipt.gasUsed / objTransaction.gas) * 100,
        fee:
          objTransactionReceipt.gasUsed *
          this.web3
            .fromWei(objTransaction.gasPrice.toString())
            .toLocaleString("en-US"),
      },
      block: {
        number: objTransaction.blockNumber
          ? objTransaction.blockNumber.toLocaleString("en-US")
          : null,
        ago: (this.intBlockNumber - objTransaction.blockNumber).toLocaleString(
          "en-US"
        ),
      },
    ...
    objTxDetails.contract.is =
      this.web3.eth.getCode(strToAddress) === "0x" ? false : true;
    ...
    objLabelledAddress = await objLabels.getLabelForAddress(
      objTransaction.from
    );
    if (typeof objLabelledAddress !== "undefined") {
      strLabel = `${objLabelledAddress.label} (${ChainLookup.getShortAddress(
        objTransaction.from
      )})`;
      objLabelledAddresses.from = objLabels.getTemplate(
        strLabel,
        objLabelledAddress.color
      );
      blUseLables = true;
    ...
    if (typeof objLabelledAddress !== "undefined") {
      ...
      objLabelledAddresses.to = objLabels.getTemplate(
        strLabel,
        objLabelledAddress.color
      );
      blUseLables = true;
                ...
                <strong>Transaction:</strong> <br />
                    <div style="text-align:center">
                        ...
                        <span>
                            ...
                            <a target="_blank" href="${
                              this.strBlockExplorer + "/" + objTransaction.from
                            }">${objLabelledAddresses.from}</a>
                        </span> 
                        ${blUseLables ? `<br />&darr;<br />` : `&#8594;`}
                        <a style="border-bottom:1px dotted #000;" title="Ξ${
                          objTxDetails.eth_full
                        }">${objTxDetails.eth} ETH</a>
                        ${blUseLables ? `<br />&darr;<br />` : `&#8594;`}
                        <span>
                            <img class="blockie" src="${strAddressToBlockie}" />
                            <a target="_blank" href="${
                              this.strBlockExplorer + "/" + strToAddress
                            }">${objLabelledAddresses.to}</a>
                        </span>
                    ...
                    <ul>
                        <li><strong>Gas Limit:</strong> ${
                          objTxDetails.gas.limit
                        } 
                        ${
                          objTransactionReceipt.is_pending
                            ? ``
                            : `(${Math.ceil(
                                objTxDetails.gas.used_percent
                              )}% consumed)`
                        }</li>
                        <li><strong>Transaction Fee:</strong> Ξ${
                          objTxDetails.gas.fee
                        } 
                            ${
                              objTransactionReceipt.contractAddress === null &&
                        ...
                        ${
                          objTransactionReceipt.is_pending
                            ? `<i>Pending</i>`
                            : `${objTxDetails.block.number} (${
                                objTxDetails.block.ago.replace(",", "") <=
                                3000000
                                  ? objTxDetails.block.ago + " blocks ago"
                                  : "millions of blocks ago"
                              })`
                        }</li>
                    </ul>
            `;
  ...
  static getShortAddress(strAddress) {
    return `${strAddress.slice(0, 4)}...${strAddress.slice(
      strAddress.length - 4,
      strAddress.length
    )}`;
  }
