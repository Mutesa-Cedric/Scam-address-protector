...
const EXT_PREFIX = 'ext-smartaddressprotection';
...
class ScamAddressProtection {
    ...
    setSearchAndReplaceSettings()
    {
        ...
        this.replacePatterns = [
            ...
            'data-address="$2"' +
            'class="ext-smartaddressprotection-link ext-smartaddressprotection-0xaddress" ' +
            'target="'+ this.target +'">' +
            '<div class="ext-smartaddressprotection-blockie" data-ether-address="$2" ></div> $2' +
            '</a>$3',

            // ENS Address Replace
            '$1<a title="See this address on the blockchain explorer" ' +
            'href="' + this.strBlockchainExplorer + '/$2" ' +
            'class="ext-smartaddressprotection-link" ' +
            ...
            'class="ext-smartaddressprotection-warning">$2</slot>$3'
        ];
    }
    ...
    setWarningSettings()
    {
        ...
        if(!this.ENSCompatiable){
            this.replacePatterns[1] = '$1<a title="Notification! We have spotted an ENS address, your current block explorer can\'t parse this address. Please choose a compatible block explorer." ' +
                'class="ext-smartaddressprotection-link ext-smartaddressprotection-warning" href="#">$2</a>$3';
        }
    }
        ...
        for(var i=0; i < objNode.childNodes.length; i++){
            ...
            if(objNode.childNodes[i].nodeType == 3){ // nodeType 3 = a text node
                ...
                if(this.isPatternMatched(childContent)) {
                    ...
                    replacement.setAttribute('class', 'ext-smartaddressprotection-temporary');
    ...
    tidyUpSlots()
    {
        var slots = document.querySelectorAll("slot.ext-smartaddressprotection-temporary");
    ...
    addBlockies()
    {
        var blockieDivs = document.querySelectorAll("div.ext-smartaddressprotection-blockie");
    ...
    {
        var objEtherAddresses = document.getElementsByClassName("ext-smartaddressprotection-link");
        for (var i = 0; i < objEtherAddresses.length; i++) {
            objEtherAddresses[i].classList.add("ext-smartaddressprotection-link-no_highlight");
            objEtherAddresses[i].classList.add("ext-smartaddressprotection-link-highlight");
        }
        return false;
    }
    ...
    addHighlightStyle()
    {
        ...
        for (var i = 0; i < objEtherAddresses.length; i++) {
            objEtherAddresses[i].classList.add("ext-smartaddressprotection-link-highlight");
            objEtherAddresses[i].classList.remove("ext-smartaddressprotection-link-no_highlight");
        }
        return false;
    }
    ...
    {
        var objNodes = document.getElementsByClassName("ext-smartaddressprotection-0xaddress");
        ...
        if (addressElement.children.length > 1 && addressElement.children[1].className === HOVER_POPUP_CLASS_NAME) {
            ...
            objBrowser.runtime.sendMessage({ func: "rpc_provider" }, (objResponse) => {
                ...
                web3.eth.getTransactionCount(str0xAddress, function(error, result) {
                    if(objHoverNodeContent.children[0].id == "ext-smartaddressprotection-fetching_data_"+uniqueAddressId) {
                    ...
                    if(error) {
                        ...
                        objHoverNodeContent.querySelector("#ext-smartaddressprotection-address_stats_hover_node_error_"+uniqueAddressId).style.display = "inline";
                        objHoverNodeContent.querySelector("#ext-smartaddressprotection-address_stats_hover_node_error_"+uniqueAddressId).innerText = "There were RPC errors";
                    } else {
                        objHoverNodeContent.querySelector("#ext-smartaddressprotection-address_stats_hover_node_ok_"+uniqueAddressId).style.display = "inline";
                        objHoverNodeContent.querySelector("#ext-smartaddressprotection-address_stats_hover_node_ok_"+uniqueAddressId).innerText = "RPC calls successful";
                        intTransactionCount = parseInt(result).toLocaleString();
                    }

                    var objTransactionCount = objHoverNodeContent.querySelector("#ext-smartaddressprotection-transactions_out_"+uniqueAddressId);
                ...
                web3.eth.getBalance(str0xAddress, function(error, result) {
                    ...
                    var objAddressBalance = objHoverNodeContent.querySelector("#ext-smartaddressprotection-address_balance_"+uniqueAddressId);
                ...
                web3.eth.getCode(str0xAddress, function(error, result) {
                    ...
                    var objContractAddress = objHoverNodeContent.querySelector("#ext-smartaddressprotection-contract_address_"+uniqueAddressId);
