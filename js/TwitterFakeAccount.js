class TwitterFakeAccount
{
    ...
    {
        var objNodes = document.getElementsByClassName("ext-smartaddressprotection-tweet-" + objData.tweet_id);
        for (var intCounter = 0; intCounter < objNodes.length; intCounter++) {
            var objNode = objNodes[intCounter];
            if (objNode.getAttribute("ext-smartaddressprotection-twitterflagged")) {
                return;
            }
            ...
            objNode.setAttribute("ext-smartaddressprotection-twitterflagged", 1);
            ...
            if(document.activeElement.classList.contains("tweet-box") === false) {
                if (document.getElementsByClassName("tweet")) {
                    ...
                    for(var intCounter=0; intCounter<arrTweets.length; intCounter++) {
                        ...
                        if(arrTweets[intCounter].classList.contains("ext-smartaddressprotection-tweet-"+arrTweets[intCounter].getAttribute("data-tweet-id")) === false) {

                            arrTweets[intCounter].classList.add("ext-smartaddressprotection-tweet-" + arrTweets[intCounter].getAttribute("data-tweet-id"));
