...
let LOCALSTORAGE_IDENTIFER = {
  DETAILS: "ext-smartaddressprotection-rpc_node_details",
  ENDPOINT: "ext-smartaddressprotection-rpc_node",
};
...
class RpcNodeSelector {
  ...
  static getCurrentNetworkDescription() {
    if (localStorage.getItem(LOCALSTORAGE_IDENTIFER.DETAILS)) {
      let objDetails = JSON.parse(
        localStorage.getItem(LOCALSTORAGE_IDENTIFER.DETAILS)
      );
      document.getElementById(
        "ext-smartaddressprotection-rpcnode_current_details"
      ).innerText = `You are currently connected to: ${[
        objDetails.name,
        objDetails.type,
      ].join(" ")}`;
    }
  ...
  populateRpcNodeInput() {
    objBrowser.runtime.sendMessage(
      ...
      function (objResponse) {
        document.getElementById(
          "ext-smartaddressprotection-rpcnode_modify_url"
        ).value = objResponse.resp;
      }
    );
  ...
  saveFormValues(objEvent) {
    var objRpcValue = document.getElementById(
      "ext-smartaddressprotection-rpcnode_modify_url"
    );

    document
      .getElementById("ext-smartaddressprotection-rpcnode_connected_status")
      .classList.add("hide-me");
    document
      .getElementById("ext-smartaddressprotection-rpcnode_details")
      .classList.add("hide-me");

    var objRpcSuccessNode = document.getElementById(
      "ext-smartaddressprotection-rpcnode_success"
    );
  ...
  static rpcEndpointIsAvailable(strHttpProvider) {
    ...
    var objRpcErrorNode = document.getElementById(
      "ext-smartaddressprotection-rpcnode_errors"
    );
  ...
  static updateNodeDetails() {
    ...
    document.getElementById(
      "ext-smartaddressprotection-rpcnode_connected_status"
    ).innerHTML = (blConnected ? "CONNECTED" : "DISCONNECTED") + "<br />";
    if (blConnected) {
      document.getElementById(
        "ext-smartaddressprotection-rpcnode_details"
      ).innerHTML = `
    ...
    let arrNetwork = ETH_NETWORK_MAP.filter(
      (v) => v.network_id === intNetworkId
    );
    document.getElementById(
      "ext-smartaddressprotection-rpcnode_details"
    ).innerHTML += `
    ...
    localStorage.setItem(
      LOCALSTORAGE_IDENTIFER.DETAILS,
      JSON.stringify(arrNetwork[0])
    );
  }
}
...
(function () {
  ...
  var objForm = document.getElementById(
    "ext-smartaddressprotection-rpcnode_modify_form"
  );
