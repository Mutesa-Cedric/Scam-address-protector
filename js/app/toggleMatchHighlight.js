const HIGHLIGHT_CHECKBOX_SELECTOR =
  '[name="ext-smartaddressprotection-show_style"]';
...
function toggleMatchHighlight() {
  ...
  localStorage.setItem(
    "ext-smartaddressprotection-show_style",
    intShowHighlight
  );
...
function refreshHighlightOption() {
  ...
  var intShowHighlight = localStorage.getItem(
    "ext-smartaddressprotection-show_style"
  );

  document.querySelector(HIGHLIGHT_CHECKBOX_SELECTOR).checked =
    intShowHighlight == 1 ? true : false;
  //Notify the tab to do a class method
  var strMethod =
    intShowHighlight == 1 ? "addHighlightStyle" : "removeHighlightStyle";
