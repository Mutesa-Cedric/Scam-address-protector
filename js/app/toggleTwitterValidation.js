const TWITTER_CHECKBOX_SELECTOR =
  '[name="ext-smartaddressprotection-twitter_validation"]';
...
function toggleTwitterValidation() {
  ...
  localStorage.setItem(
    "ext-smartaddressprotection-twitter_validation",
    intTwitterValidation
  );
...
function refreshTwittertOption() {
  ...
  if (
    localStorage.getItem("ext-smartaddressprotection-twitter_validation") ===
    null
  ) {
    intTwitterValidation = true;
  } else {
    intTwitterValidation = localStorage.getItem(
      "ext-smartaddressprotection-twitter_validation"
    );
  }
  ...
  document.querySelector(TWITTER_CHECKBOX_SELECTOR).checked =
    intTwitterValidation == 1 ? true : false;
}
...
function getTwitterLists() {
  ...
  if (localStorage.getItem("ext-smartaddressprotection-twitter_lists")) {
    let saved_settings = JSON.parse(
      localStorage.getItem("ext-smartaddressprotection-twitter_lists")
    );
  ...
  if (Math.floor(Date.now() - twitter_lists.last_fetched) > 600 * 1000) {
    fetch(
      "https://raw.githubusercontent.com/MrLuit/EtherScamDB/master/_data/twitter.json"
    )
      .then((res) => res.json())
      .then((lists) => {
        ...
        localStorage.setItem(
          "ext-smartaddressprotection-twitter_lists",
          JSON.stringify(twitter_lists)
        );
      });
  }
}
...
window.addEventListener("load", function () {
  document
    .querySelector(TWITTER_CHECKBOX_SELECTOR)
    .addEventListener("click", toggleTwitterValidation);
  refreshTwittertOption();
});
