const ETHEREUM_NETWORK_IDS = {
  //As per https://ethereum.stackexchange.com/a/17101/8198
  0: "Olympic",
  1: "Mainnet",
  2: "Morden",
  3: "Ropsten",
...
function toggleBlockchainExplorer() {
  var objBlockchainExplorer = document.getElementById(
    "ext-smartaddressprotection-choose_blockchain"
  );
  localStorage.setItem(
    "ext-smartaddressprotection-blockchain_explorer",
    objBlockchainExplorer.options[objBlockchainExplorer.selectedIndex].value
  );

  // See if the node is on a different network
  let intNetworkId =
    objBlockchainExplorer.options[objBlockchainExplorer.selectedIndex].dataset
      .network;
  ...
  chrome.runtime.sendMessage({ func: "rpc_provider" }, (objResponse) => {
    ...
    if (intWeb3NetworkId !== intNetworkId) {
      document.querySelector(
        ".ext-smartaddressprotection-note_network_diff"
      ).style.display = "block";
      document.querySelector(
        ".ext-smartaddressprotection-note_network_diff"
      ).innerHTML = `
...
function refreshBlockchainExplorer() {
  var strBlockchainExplorer = localStorage.getItem(
    "ext-smartaddressprotection-blockchain_explorer"
  );
  ...
  if (strBlockchainExplorer === null) {
    document.getElementById(
      "ext-smartaddressprotection-choose_blockchain"
    ).value = "https://etherscan.io/address";
  } else {
    document.getElementById(
      "ext-smartaddressprotection-choose_blockchain"
    ).value = strBlockchainExplorer;
  }
