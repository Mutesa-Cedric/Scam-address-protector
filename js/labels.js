...
const FORM_ADDRESS_SELECTOR = "#ext-smartaddressprotection-label-address";
const FORM_NAME_SELECTOR = "#ext-smartaddressprotection-label-name";
const FORM_COLOR_SELECTOR = "#ext-smartaddressprotection-label-color";
...
class Labels {
  ...
  shortenAddress(address) {
    ...
    const end = address.slice(
      address.length - BEGINNING_AND_END_CHARS_IN_ADDR_TO_SHOW,
      address.length
    );
  ...
  getTemplate(name, color) {
    ...
    return `<span
            class='ext-smartaddressprotection-label'
  ...
  getExtendedTemplate(address, name, color) {
    ...
    return `<span
            ...
            &nbsp;
            <span style="float:right; cursor:pointer;" class="ext-smartaddressprotection-label-delete" data-ext-smartaddressprotection-label-id="${address}">x</span>
            <br/>`;
  }
  ...
  addLabelsListEvents() {
    const labelsDeleteElements = document.getElementsByClassName(
      "ext-smartaddressprotection-label-delete"
    );

    Array.from(labelsDeleteElements).forEach((element) => {
      element.addEventListener("click", async (event) => {
        const id = event.target.getAttribute(
          "data-ext-smartaddressprotection-label-id"
        );
    ...
    document
      .querySelectorAll(`[${FILL_LABEL_INPUT_ATTRIBUTE}]`)
      .forEach((element) => {
        element.addEventListener("click", async (event) => {
  ...
  setupFormSubmitHandler() {
    document
      .getElementById("ext-smartaddressprotection-new-label-form")
      .addEventListener("submit", async (event) => {
        ...
        if (!name || !address || !color) {
          alert(
            'Please make sure that "Name", "Address", and "Color" is filled.'
          );
        } else {
  ...
  async updateLabelsList() {
    ...
    for (const [address, label, color] of retrievedLabels) {
      HTMLLabels += this.getExtendedTemplate(address, label, color);
    }

    document.getElementById(
      "ext-smartaddressprotection-current-labels"
    ).innerHTML = HTMLLabels;
